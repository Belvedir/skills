...
## First, inventory the LLM call sites

Real codebases hide LLM calls outside the obvious client factory, and a missed call site produces no error, just silently missing data. Before wiring anything, enumerate every place the code talks to a model: official SDK clients (including ones pointed at OpenRouter/vLLM/proxies: auto-captured regardless of base URL, since instrumentation patches the client, not the destination), raw-HTTP calls, embeddings, provider batch-API usage, one-off side calls (titling, classification), and background workers or cron scripts (separate processes; each needs its own `initialize()`). Greps that find most: `OpenAI(`, `Anthropic(`, `chat/completions`, `/v1/messages`, `api.openai.com`, `api.anthropic.com`, `openrouter.ai`, `embeddings`. Know the capture boundary: raw-HTTP capture watches only `POST .../chat/completions` (Node `fetch` 0.4.0+, Python `requests`/`httpx` 0.6.0+, streams included); raw HTTP to Anthropic's `/v1/messages`, to `/embeddings` or `/responses`, and Python `aiohttp` are NOT captured; move those call sites to the official client SDKs, which always are.
...
**Background workers and threads:** `initialize()` is per process: call it at each worker's startup (after the fork on prefork servers: an exporter thread started before `fork()` doesn't survive into the child), and note `flush()` silently no-ops in a process that never initialized. The session context is contextvars-based: it follows `await` but never crosses a thread, process, or queue boundary (Celery, Temporal, `multiprocessing`, thread pools). Pass the session id in the job payload and re-open `session(session_id=...)` inside the worker; spans under the same id merge into one session. `flush()` is synchronous, so calling it immediately before `os._exit()` is safe; the hazard is exiting without it. Don't nest `session()` calls (an inner session replaces the outer context), and don't put keys named `session_id`/`user_id` in `metadata` (they'd clobber the real ones).
...
| `instrumentModules` (Node only) | none | Your imported LLM SDKs, e.g. `{ openAI: openai.OpenAI, anthropic }`; patches them directly. Required in Next.js and every ESM/bundled environment |
...
- **No spans at all**: wrong `baseUrl` (silent 404), `initialize()` ran after the LLM SDK was imported (Node logs a late-init warning), an ESM entry point without `instrumentModules` (no warning; the require hook never sees ESM imports), or the route runs on the Edge runtime (common on Vercel: add `export const runtime = "nodejs"` to the route).
- **Traces silently missing on a legacy package**: Node `@fractalresearch/loop` before 0.1.5 defaults to the retired `platform.fractalresearch.ai` ingest host and drops spans silently (every published Python release already targets the live host). Upgrade to `belvedir`, or pass `baseUrl: "https://platform.belvedir.ai"` explicitly.
